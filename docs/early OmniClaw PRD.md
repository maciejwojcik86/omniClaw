...
/home/agent\_xyz/workspace/  
 ├─ inbox/unread/         (Kernel drops incoming YAML/MD emails here)  
 ├─ inbox/read/           (Agent moves emails here after processing)  
 ├─ outbox/pending/       (Agent writes YAML/MD emails here to be routed)  
 ├─ outbox/drafts/        (Working folder with drafts, move to pending only once agent finished working on draft)
 ├─ outbox/sent/          (Kernel moves routed emails here)  
 ...
 ├─ journal/              (DAILY-YYYY-MM-DD.md: short run log \+ reflection)  
 ├─ metrics/KPI.csv       (Rolling self-metrics)  
 ├─ drafts/               (WIP code/outputs before QA submission)  
 ├─ persona\_template.md   (Authored by HR/Manager, contains {{variables}})  
 └─ AGENTS.md             (Read-only final prompt, auto-rendered by Kernel)
...
### **3.4 Formal Communication (Stateful Forms)**
...
form\_id: REQ-2026-004  
type: budget\_increase  
status: submitted  
sender: Agent\_Dev\_02  
target: Agent\_Manager\_01  
requested\_amount: 5.00  
justification: "Require funds for deep data extraction on Project Alpha."

* **Workflow Loop:** The Manager reviews it. If inadequate, the Manager edits the status to returned\_for\_revision, adds comments in the Markdown body, and puts it in their outbox. The Kernel routes it back. The database tracks every state change.
...
* \[ \] Implement Linux OS interactions: Scripts to run useradd, generate keys, and set up the exact workspace directory structure (inbox, outbox, notes, journal, drafts, AGENTS.md).  
