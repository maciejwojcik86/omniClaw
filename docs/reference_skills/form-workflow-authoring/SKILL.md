...
- Kernel reads `form_type`, `stage`, `decision` and uses active `workflow_graph` to resolve next stage/holder.
...
- `decision`: explicit decision from current stage decisions (mandatory; no implicit fallback)

### Common optional fields
- `target`: required when next stage target is dynamic (`{{any}}` / `{{var}}`)
- `form_id`: present after first successful decision
...
- Kernel validates `decision` against current-stage decisions in active `workflow.json`.
...
- Which `decision` values are legal in this stage
...
- [ ] every non-terminal stage has decisions
