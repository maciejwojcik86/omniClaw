...
**Reason**: AGENT nodes no longer rely on distinct Linux users and manager-group filesystem permissions as the canonical provisioning model.

**Migration**: Provision AGENT nodes into repo-local Nanobot directories under `workspace/agents/<agent_name>/`; retain legacy permission scripts only for historical Nullclaw paths and non-canonical manual operations.
