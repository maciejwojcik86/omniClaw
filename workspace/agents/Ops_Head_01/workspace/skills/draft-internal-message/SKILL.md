...
## Steps
1. Copy template `templates/message_draft.md` into `<workspace>/outbox/send/<file>.md`.
2. Fill frontmatter:
   - `form_type: message`
   - `stage: DRAFT`
   - `decision: send`
...
4. Leave the file in `outbox/send/` for kernel scan.
...
## Fallback
- If IPC reports `undelivered`, fix frontmatter error and keep file in `outbox/send/`.
