...
- local operators can start the stack with `uv run omniclaw` (this also auto-starts the local LiteLLM proxy when `LITELLM_PROXY_URL` targets loopback)
...
- Confirm `show_session_cost.py` reports non-zero calls/tokens/cost for a completed session key.

## Fallback

- If the helper reports that the kernel is unreachable, start OmniClaw with `uv run omniclaw` and rerun the budget action.
