...
def company_workspace_root() -> Path:
    explicit = os.getenv("OMNICLAW_COMPANY_WORKSPACE_ROOT", "").strip()
    if explicit:
        return Path(explicit).expanduser().resolve()

    config_path = Path(
        os.getenv("OMNICLAW_GLOBAL_CONFIG_PATH", str(Path.home() / ".omniClaw" / "config.json"))
    ).expanduser().resolve()
    company_ref = os.getenv("OMNICLAW_COMPANY", "").strip()
    if config_path.exists():
        try:
            payload = json.loads(config_path.read_text(encoding="utf-8"))
            companies = payload.get("companies")
            if isinstance(companies, dict):
                if company_ref and company_ref in companies and isinstance(companies[company_ref], dict):
                    workspace_root = companies[company_ref].get("workspace_root")
                    if workspace_root:
                        return Path(str(workspace_root)).expanduser().resolve()
                if company_ref:
                    lowered = company_ref.casefold()
                    matches = [
                        value
                        for value in companies.values()
                        if isinstance(value, dict)
                        and str(value.get("display_name", "")).strip().casefold() == lowered
                    ]
                    if len(matches) == 1:
                        workspace_root = matches[0].get("workspace_root")
                        if workspace_root:
                            return Path(str(workspace_root)).expanduser().resolve()
                if len(companies) == 1:
                    only = next(iter(companies.values()))
                    if isinstance(only, dict) and only.get("workspace_root"):
                        return Path(str(only["workspace_root"])).expanduser().resolve()
        except (OSError, json.JSONDecodeError):
            pass
    return Path.home().joinpath(".omniClaw", "workspace").expanduser().resolve()


def template_root() -> Path:
    root = company_workspace_root() / "nanobot_workspace_templates"
    if root.exists():
        return root
    repo_root = find_repo_root(Path(__file__).resolve().parent)
    if repo_root is None:
        raise FileNotFoundError("Unable to locate company workspace or OmniClaw repo template root")
