...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Decision key for decision field (default: acknowledge_read).",
    )
    parser.add_argument(
        "--to-status",
        default="ARCHIVED",
        help="Target status for decision update (default: ARCHIVED).",
    )
    parser.add_argument(
        "--apply",
        action="store_true",
...
def remove_pair(pairs: list[tuple[str, str]], key: str) -> None:
    ...
    try:
        ...
        if (
            ...
            if fallback_actor_node_id:
                retry_payload = _build_decision_payload(
                    form_id=form_id,
...
def main() -> int:
    ...
    upsert_pair(pairs, "decision", args.decision_key)
    remove_pair(pairs, "transition")
