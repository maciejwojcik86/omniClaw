...
def template_root() -> Path:
    ...
    if repo_root is None:
        raise FileNotFoundError("Unable to locate OmniClaw repo root for workspace/agent_templates")
    root = repo_root / "workspace" / "agent_templates"
    if not root.exists():
        raise FileNotFoundError(f"Agent template root not found: {root}")
    return root
