...
3. `skills/deploy-new-nanobot/scripts/create_workspace_tree.py` (write the OmniClaw/Nanobot workspace scaffold from `/home/macos/omniClaw/workspace/agent_templates/`)
4. `skills/deploy-new-nanobot/scripts/init_nanobot_config.py` (write or update the sibling `config.json` from `/home/macos/omniClaw/workspace/agent_templates/config.json`)
5. `skills/deploy-new-nanobot/scripts/write_agent_instructions.py` (write workspace-root `AGENTS.md` from `/home/macos/omniClaw/workspace/agent_templates/AGENTS.md`)
...
- keep `decision` empty
- leave the form in `inbox/unread/` until the blocker is fixed

Only route the form to `outbox/pending/` when deployment and smoke verification have succeeded and you can truthfully set `decision: deploy_and_archive`.
