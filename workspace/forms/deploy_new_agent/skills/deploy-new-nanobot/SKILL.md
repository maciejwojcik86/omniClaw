...
- Write or update the sibling Nanobot runtime config at `workspace/agents/<agent_name>/config.json` using the canonical template at `/home/macos/omniClaw/workspace/agent_templates/config.json`.
- Write workspace-root `AGENTS.md` instructions and preserve the Nanobot-native context files using the canonical files under `/home/macos/omniClaw/workspace/agent_templates/`.
...
2. Re-run `scripts/init_nanobot_config.py --apply` for any actor whose config drifted from the approved baseline.
   The baseline template lives at `/home/macos/omniClaw/workspace/agent_templates/config.json`.
...
- Keep `decision` empty.
- Leave the form in `inbox/unread/` for retry or human remediation; do not move it to `/outbox/pending/`.
...
- Save the updated form to `/outbox/pending/`.
