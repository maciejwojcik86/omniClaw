# Troubleshooting: Deploy New Nanobot Standalone
...
```bash
python3 skills/deploy-new-nanobot-standalone/scripts/init_nanobot_config.py --apply --workspace-root /home/macos/omniClaw/workspace/agents/<agent_name>/workspace --config-path /home/macos/omniClaw/workspace/agents/<agent_name>/config.json
...
```bash
python3 skills/deploy-new-nanobot-standalone/scripts/create_workspace_tree.py --apply --workspace-root /home/macos/omniClaw/workspace/agents/<agent_name>/workspace
python3 skills/deploy-new-nanobot-standalone/scripts/init_nanobot_config.py --apply --workspace-root /home/macos/omniClaw/workspace/agents/<agent_name>/workspace --config-path /home/macos/omniClaw/workspace/agents/<agent_name>/config.json
```
...
Symptom:
- Workspace scaffold or config generation fails because `workspace/nanobot_workspace_templates/*` is missing or stale.

Cause:
- The deploy skill and kernel scaffold both load standard files from `/home/macos/omniClaw/workspace/nanobot_workspace_templates/`.

Fix:
- Restore the expected files under `/home/macos/omniClaw/workspace/nanobot_workspace_templates/`.
- Re-run `skills/deploy-new-nanobot-standalone/scripts/create_workspace_tree.py` and `skills/deploy-new-nanobot-standalone/scripts/init_nanobot_config.py` after the templates are corrected.
