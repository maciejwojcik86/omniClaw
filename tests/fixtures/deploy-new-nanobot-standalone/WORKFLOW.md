# Workflow: Deploy New Nanobot Standalone

This is the direct/manual deployment path. Use the form-linked `deploy-new-nanobot` skill when deployment must remain inside the approved `deploy_new_agent` workflow.
...
- optional: `seed_config` (`skills/deploy-new-nanobot-standalone/templates/nanobot_seed_config.json`)
...
1. optional `skills/deploy-new-nanobot-standalone/scripts/package_nanobot_source.sh` (archive the vendored `third_party/nanobot` fork)
2. `skills/deploy-new-nanobot-standalone/scripts/trigger_kernel_action.sh` with action `provision_agent`
3. `skills/deploy-new-nanobot-standalone/scripts/create_workspace_tree.py` (write the OmniClaw/Nanobot workspace scaffold from `/home/macos/omniClaw/workspace/nanobot_workspace_templates/`)
4. `skills/deploy-new-nanobot-standalone/scripts/init_nanobot_config.py` (write or update the sibling `config.json` from `/home/macos/omniClaw/workspace/nanobot_workspace_templates/config.json`)
5. `skills/deploy-new-nanobot-standalone/scripts/write_agent_instructions.py` (write workspace-root `AGENTS.md` from `/home/macos/omniClaw/workspace/nanobot_workspace_templates/AGENTS.md`)
6. verify workspace tree, config path, DB node linkage, and manual `nanobot gateway/agent -w -c` smoke commands

All steps are orchestrated by `skills/deploy-new-nanobot-standalone/scripts/deploy_new_nanobot.sh`.
...
- capture the failed execution report in the calling task or operator log
- do not claim deployment success until the provisioning path and smoke checks succeed
